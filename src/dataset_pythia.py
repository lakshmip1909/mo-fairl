"""
src/dataset_pythia.py

PreferenceDataset for Pythia-based reward model.

Difference from dataset.py (sentence-transformer version):
    - Does NOT pre-encode: stores raw text, tokenises in __getitem__
    - Returns input_ids + attention_mask for A and B separately
    - rho labels in {-1, +1} — same convention as dataset.py after the patch
    - Works with both real datasets (civil_comments, GSM8K, HumanEval)
      and synthetic V2 multiobjective pairs (no nulls)
"""
...
class PythiaPreferenceDataset(Dataset):
    """
    Each item returns:
        input_ids_a      : LongTensor  [seq_len]
        attention_mask_a : LongTensor  [seq_len]
        input_ids_b      : LongTensor  [seq_len]
        attention_mask_b : LongTensor  [seq_len]
        rho              : FloatTensor [K]   {-1.0, +1.0}
    ...
    def _parse_rho(raw_labels: dict) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Convert {0,1,null} labels → rho in {-1.0, +1.0} with mask.
        Identical convention to dataset.py after the rho patch.
        """
        rho  = []
        mask = []
    ...
    def _tokenise(self, text: str) -> dict[str, torch.Tensor]:
        enc = self.tokenizer(
            text,
            padding      = "max_length",
            truncation   = True,
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        tok_a = self._tokenise(text_a)
        tok_b = self._tokenise(text_b)
...
def pythia_collate_fn(batch: list[dict]) -> dict:
    return {
...
def build_pythia_dataloaders(
    ...
    dataset = PythiaPreferenceDataset(jsonl_path, tokenizer_name=tokenizer_name)
    n       = len(dataset)
    ...
    kwargs = dict(collate_fn=pythia_collate_fn, num_workers=2, pin_memory=True)
